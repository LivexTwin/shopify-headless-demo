...
<ul class="space-y-6">
  {#each cart.lines as line, index (line.id)}
    <li
      class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-gray-50 p-4 rounded-xl shadow-sm"
    >
      <div class="flex gap-4 items-start sm:items-center">
        <a href={`/products/${line.merchandise.product.handle}`}>
          <ShopifyImage
            image={normalizeProductImage(line.merchandise.product) ?? null}
            loading={index === 0 ? 'eager' : 'lazy'}
            classList="rounded-md w-full h-full object-cover"
          />
        </a>
        ...
        <div>
          ...
          <p class="text-sm text-gray-600">
            Options: {line.merchandise.title}
          </p>
      ...
      <div class="flex items-center gap-2 mt-2 sm:mt-0">
        ...
        <label class="sr-only" for={`quantity-${line.id}`}>
          Quantity
        </label>
